...
// Mobile Card View
function StudentCard({ student, index, onEdit, onDelete, onGenerateQR }: {
  student: Student;
  index: number;
  onEdit: (student: Student) => void;
  onDelete: (student: Student) => void;
  ...
  const photoUrl = student.photo_path ? studentsService.getPhotoUrl(student.id) : undefined;
        ...
        <div className="flex items-center justify-end gap-2 mt-3 pt-3 border-t">
          {student.barcode_token && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(studentsService.getQRDownloadUrl(student.id), '_blank')}
              className="gap-1.5"
            >
          ...
          {!student.barcode_token && onGenerateQR && (
            <Button
              variant="outline"
              size="sm"
...
export function StudentTable({ students, onEdit, onDelete, onGenerateQR, isLoading }: StudentTableProps) {
